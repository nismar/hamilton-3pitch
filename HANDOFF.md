...
- **After Sept 12–13, 2026:** enter the real tournament pool finishes and bracket results (currently the Sunday side is placeholders). Pool finishes are ranked by record → run differential capped at ±10/game → regular-season seed (`tournament.poolRanking` in `data.js`). Saturday runs must be recorded at the diamond for that tiebreak, but **runs still never go on the site** — publish only the resulting A1–B4 order. Saturday pool games render as "lower seed at higher seed" (higher seed = home).
